...
const reseñasPorDefecto = (ProductoID: number) =>`
...
const reseñasAsc = (ProductoID: number) => `
...
const reseñasDesc = (ProductoID: number) => `
SELECT reseñas.ProductoID, us.AliasUsuario, reseñas.Puntuacion, reseñas.Cuerpo, reseñas.FechaEmision
FROM Usuario us RIGHT JOIN
(SELECT ProductoID, UsuarioID, Puntuacion, Cuerpo, FechaEmision
FROM Puntuacion punt LEFT JOIN Comentario com
...
const existeElAlias = (AliasAComprobar: string) => `SELECT AliasUsuario FROM Usuario WHERE AliasUsuario = ${AliasAComprobar};`
const existeElMail = (MailAComprobar: string) => `SELECT Email FROM Usuario WHERE Email = ${MailAComprobar};`
...
const registrar = (Alias: string, Nombres: string, Apellido: string, FechaNac: Date, Edad: number, Email: string, Telefono: string, FechaDeRegistro: Date, ContraseñaHasheada: string) => `
BEGIN;
INSERT INTO Usuario 
...
const existeElUsuario = (AliasUsuario: string) => `
...
const devolverJWTData = (AliasUsuario: string) => `
...
// obtener mis datos
const misDatos = (UsuarioID: number, AliasUsuario: string, Nombres: string, Apellido: String) => `
...
NAlias: string, NNombres: string, NApellido: string, NFechaDeNacimiento: Date, NEdad: number, NEmail: string, NTelefono: string ) => `
BEGIN;
...
// obtener carrito
const miCarrito = (UsuarioID: number, AliasUsuario: string, Nombres: string, Apellido: String) => `
...
const totalMiCarrito = (UsuarioID: number, AliasUsuario: string, Nombres: string, Apellido: String) => `
...
const agregarAlCarrito = (UsuarioID: number) => `
BEGIN;
...
// vaciar carrito
const vaciarMiCarrito = (UsuarioID: number) => `
BEGIN;
...
const miReseña = (UsuarioID: number, AliasUsuario: string, Nombres: string, Apellido: String, ProductoID: number) => `
...
const existeLaPuntuacion = (UsuarioID: number, ProductoID: number) => `SELECT Puntuacion FROM Puntuacion WHERE UsuarioID = ${UsuarioID} AND ProductoID = ${ProductoID};`

// si no existe (porque la consulta anterior devolvio consulta vacia) lo insertamos:
const insertarPuntuacion = (UsuarioID: number, ProductoID: number, Puntuacion: number) => `
BEGIN;
...
const modificarPuntuacion = (UsuarioID: number, ProductoID: number) => `
BEGIN;
...
const validacionComentario = (UsuarioID: number, ProductoID: number) => `
...
const comentarioNuevo = (UsuarioID: number, ProductoID: number, Comentario: string) => `
BEGIN;
...
const modificarComentario = (UsuarioID: number, ProductoID: number, Comentario: string) => `
BEGIN;
